...
import numpy as np
from PionScatLib import Tlab2Wcm, getGH
...
def main():
    ...
    topStr = "  Theta     DSG             My f            My g/sin(theta)            ron_f/my_f"
    topStr += "         ron_g/my_g        abs(f ratio)      abs(g ratio)"
    reactions = ["pi+p", "pi-p"]
    ...
    for reaction in reactions:
        ...
        print(
            "  Theta      DSG               f               g/sin(theta)     DSG_ron/DSG_me"
        )
        for i, theta in enumerate(thetas):
            ...
            myDSG = abs(f) ** 2 + abs(g * np.sin(theta * np.pi / 180)) ** 2
            myDSG = myDSG * 10
            f = fs[i]
            g = gs[i]
            ratio = DSG / myDSG
