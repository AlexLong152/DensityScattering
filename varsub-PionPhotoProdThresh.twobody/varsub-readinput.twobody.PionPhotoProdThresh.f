...
c     
      ...
      integer,intent(out) :: j12max
      integer,intent(out) :: numDiagrams                   !along April 2023: total number of diagrams to be used 
...
c     
      ...
      if (index(calcstring,'numDiagrams=').eq.0) then
         numDiagrams = 1             
         write(*,'(A,I4)') " Number of diagrams not specified -- using default numDiagrams =",numDiagrams
         write(*,*) "calcstring="
         write(*,*) calcstring
      else if (index(calcstring,'numDiagrams=').ne.0) then
         if (index(calcstring,'numDiagrams=1').ne.0) then
            numDiagrams = 1
         else  if (index(calcstring,'numDiagrams=2').ne.0) then
            numDiagrams = 2
         else  if (index(calcstring,'numDiagrams=3').ne.0) then
            numDiagrams = 3
         else  if (index(calcstring,'numDiagrams=4').ne.0) then
            numDiagrams = 4
         else  if (index(calcstring,'numDiagrams=5').ne.0) then
            numDiagrams = 5
         else  if (index(calcstring,'numDiagrams=6').ne.0) then
            numDiagrams = 6
         else  if (index(calcstring,'numDiagrams=7').ne.0) then
            numDiagrams = 7
         else
            write(*,*) "*** ERROR: Input attempted to set numDiagrams to value greater than 7. -- Exiting."
            stop
         end if    

         write(*,'(A,I4)') "Number of diagrams set to numDiagrams=",numDiagrams
c        write(dummy,'(I1)') numDiagrams!TODO: is this right? I just copied from the above
c        descriptors = trim(descriptors) // "numDiagrams=" // dummy // "-"
      end if
c     
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc 
