...
c 1. Partial wave amplitudes (fOut in getFAtValue at PionScat.f):
c   - Calculated as (eta * exp(2i*δ) - 1) / (2i*|q|) where |q| is in MeV
c   - eta is unitless
c   - Multiplied by MeVtofm = 197.327 fm·MeV
c   - Result: units of fm
c 2. Scattering amplitudes g and h (from getGH):
c   - Built from partial wave amplitudes fPlus and fMinus
c   - Have units of fm (inherited from fOut)
c 3. Scattering matrix (Mmat from getMat):
c   - Calculated as mat = iden * g + matFactor * h (PionScat.f:139)
c   - matFactor is dimensionless (involves Pauli matrices and unit vectors)
c   - Result: units of fm
c 4. Final output (outputMat in main.onebodyvia1Ndensity.f:358):
c   - outputMat = Anucl * rho1b(rindx) * Mmat(twom1Np,twom1N)
c   - Anucl is dimensionless (nuclear mass number)
c   - rho1b is dimensionless (documented at read1Ndensity.f:11)
c   - Mmat has units of fm
c   - Result: units of fm
c   - multiply result by 10^3/(197*134) to get result in 10^-3/m_pi
...
c**********************************************************************
         do i=1,Nangles
            if (frame.eq.lab) then
              write(*,*) "lab frame not implimented"
              stop
            else
...
c           outputMat in units of fm^-1 right now
            outputMat=outputMat*HC!units of MeV now
            outputMat=outputMat*(1/(8*pi*sqrtSReal)) !no units
            outputMat=outputMat*(1/(mpi0*10**-3.d0))!10^-3 mpi^-1 units
