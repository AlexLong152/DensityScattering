...
c      logical invokesymmetryextQnum ! function to invoke symmetry/-ies of amplitudes, dependent on process
      real*8 tmpRho, ppAbs
      real*8 p12magtmp
      real*8 rho1,rho2,rho3,rho4,pAbs
...
c                          ppVecs=ppVecs        !ppVecs in units of MeV from 2Bkernel already
     ...
     &                                      mask=.not.(diffspp.eq.diffspp(locpp)))! second closest value
                               ...
                               y1=P12_MeV(locpp)
                               y2=P12_MeV(loc2pp)
c TODO: check if order matters on the points, do we require x1<x2 etc? If not then we can delete this part below.
                               ...
                               if (x1.gt.x2) then
                                   write(*,*) "In finalstatesums.twobodyvia2Ndensity.f: x1.gt.x2=",x1.gt.x2 
                                   write(*,*) "In finalstatesums.twobodyvia2Ndensity.f: x1,x2=",x1,x2 
                                   stop
                               end if
                               if (P12_MeV(locp).ne.x1) then
                                   write(*,*) "P12_MeV(locp).ne.x1" 
                                   stop
                               end if
                               if (P12_MeV(loc2p).ne.x2) then
                                   write(*,*) "P12_MeV(loc2p).ne.x2" 
                                   stop
                               end if
...
c                              if (y1.gt.y2) then
c                                  write(*,*) "In finalstatesums.twobodyvia2Ndensity.f: y1.gt.y2=",y1.gt.y2 
c                                  write(*,*) "In finalstatesums.twobodyvia2Ndensity.f: y1,y2=",y1,y2 
c                                  stop
c                              end if
c                              if (P12_MeV(locpp).ne.y1) then
c                                  write(*,*) "P12_MeV(locpp).ne.y1" 
c                                  stop
c                              end if
c                              if (P12_MeV(loc2pp).ne.y2) then
c                                  write(*,*) "P12_MeV(loc2p).ne.y2" 
c                                  stop
c                              end if
...
c   I'm going to assume pAbs and ppAbs are in MeV for now TODO: check this
...
c Implimentation of this: https://en.wikipedia.org/wiki/Bilinear_interpolation
      ...
      real*8 tmpRho2
      real*8 term1,term2 
      real*8 myMat(2,2), output(2,1)
      real*8 xVec(2),yVec(2), tmp(2)
